...
| 경로 | 용도 |
| --- | --- |
| `10-cilium-gateway-api-values.yaml` | 현재 chaining 설정을 유지하면서 Gateway API, kube-proxy replacement, EnvoyConfig, L7 proxy를 활성화했던 검증 values |
...
| `30-cilium-primary-service-mesh-values.yaml` | Cilium primary CNI 환경용 Service Mesh values 샘플 |
...
- Service Mesh values에는 구축 검증용 Hubble Relay/UI/metrics/exporter를 포함합니다. 운영 무게 비교에서는 Hubble을 Cilium Service Mesh 본체 비용에서 제외합니다.
