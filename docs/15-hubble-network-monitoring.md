...
현재 `docs/30-track-c-calico-chaining-hubble.md`로 구성한 `Calico 유지 + Cilium generic-veth chaining + Hubble` 상태와 이 문서는 정합성이 맞습니다. 운영형 Grafana 구성은 CNI를 바꾸지 않고, 현재 chaining values를 유지한 채 metrics와 `ServiceMonitor`, dashboard ConfigMap만 추가하는 방식입니다.
...
현재 Track C 기본값은 Cilium Ingress Controller를 활성화하지 않습니다. 따라서 이 절은 `nginx-ingress`, NHN Cloud ALB Controller, 또는 별도 검증된 Ingress Controller가 있을 때만 진행합니다.
...
이번 Track C에서는 Cilium 예제 `monitoring-example.yaml`보다 `kube-prometheus-stack`을 우선 사용합니다. 이유는 운영 전환 시 `ServiceMonitor`, Grafana sidecar dashboard, Prometheus target 관리 방식이 더 현실적이기 때문입니다.
...
| `manifests/15-kube-prometheus-stack-values.yaml` | Cilium/Hubble `ServiceMonitor`와 dashboard ConfigMap 수집 설정 |
| `manifests/15-cilium-hubble-metrics-values.yaml` | Track C chaining 유지 + Cilium/Hubble metrics + dashboard ConfigMap 활성화 |

전제:

- `docs/30-track-c-calico-chaining-hubble.md` 5번까지 통과
- `cilium status` 정상
...
helm upgrade --install kube-prometheus-stack prometheus-community/kube-prometheus-stack \
  ...
  --version "${KPS_VERSION}" \
  --values manifests/15-kube-prometheus-stack-values.yaml
```
...
현재 Track C chaining 설정을 유지하면서 metrics만 추가합니다.

```bash
helm upgrade cilium oci://quay.io/cilium/charts/cilium \
  --version "${CILIUM_VERSION}" \
  --namespace kube-system \
  --values manifests/15-cilium-hubble-metrics-values.yaml
```
...
이미 3-3에서 Grafana metrics를 활성화한 상태라면 `manifests/15-cilium-hubble-metrics-values.yaml`을 base로 사용합니다. 이 파일에는 Track C chaining 설정과 metrics, `ServiceMonitor`, dashboard 설정이 모두 포함되어 있습니다.
