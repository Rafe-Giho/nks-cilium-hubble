...
| 노드 | `ta-sgh-vxlan-sidecarless-cls-default-worker-node-3` / `node-4` |
| 노드 Internal IP | `192.168.200.109`, `192.168.200.104` |
...
| Cilium 버전 | `1.19.3`, Helm release `kube-system/cilium` |
| Hubble | Relay/UI 설치됨 |
...
| 서비스 메시 사용 여부 | 현재 NKS Calico chaining 환경에서는 Cilium Service Mesh 운영 불가로 판정 |
| 관측 스택 | Cilium chaining + Hubble 설치됨 |
...
| CRD | Cilium CRD 존재. Gateway API CRD는 없음 |
...
- Cilium/Hubble 운영형 모니터링에서 Prometheus/Grafana/exporter까지 포함할지 여부
- Cilium Service Mesh가 필요한 경우 Cilium primary CNI 가능 별도 환경 확보 여부
...
- 현재 문서는 2026-05-14 기준 문서 범위 정리에 맞춰 갱신했습니다.
- Cilium/Hubble은 설치되어 있고, 현재 NKS Calico chaining 환경의 Cilium Service Mesh 검증은 실패로 판정했습니다.
