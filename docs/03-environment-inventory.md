...
| 서비스 메시 사용 여부 | 미구축. Cilium Gateway API/GAMMA 별도 트랙 준비 중 |
| 관측 스택 | Cilium chaining + Hubble 설치됨. Pixie, Beyla 후보 별도 검토 |
...
- 현재 문서는 2026-05-11 `kubectl --kubeconfig` read-only 조회 결과를 기준으로 일부 갱신했습니다.
- Cilium/Hubble은 설치되어 있고, Service Mesh 관련 Gateway API CRD와 controller 활성화는 아직 수행하지 않았습니다.
