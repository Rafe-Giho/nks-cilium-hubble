...
NHN Cloud NKS에서 Calico 유지 기반 Cilium chaining + Hubble 관측 구성을 설계·검증하고, Cilium Service Mesh 적용 가능 범위와 실패 근거를 정리하기 위한 PoC 작업 공간입니다.

## 현재 상태

- `Cilium full replacement`는 실패로 종결했고, 현재 실행 경로는 `Calico 유지 + Cilium generic-veth chaining + Hubble`입니다.
- 현재 NKS Calico chaining 환경에서 Cilium Service Mesh는 운영 수준 구성 불가로 판정했습니다.
...
- 목표: Calico 유지 + Cilium chaining + Hubble Relay/UI/CLI, Grafana metrics, exporter 검증
- 추가 목표: Cilium Service Mesh 표준 구성 조건과 현재 NKS chaining 환경의 불가 사유 문서화
...
5. Cilium Service Mesh 관련 판단과 표준 가이드는 `docs/service-mesh/`에서 확인합니다.
6. 실행 결과와 실패 로그는 `artifacts/`에 남깁니다.
